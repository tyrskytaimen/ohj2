...
GameBoard::GameBoard(): board_(), splashes_() {
    ...
    // Initializing the game board in the same way as in waterdrop_game_v1, but ...
...
void GameBoard::print(std::ostream& stream) {
    // do-while is a loop that will be executed at least once. Condition is at the end
    // of the structure, and it will be evaluated after executing the body.
    do {
...
char GameBoard::droplessSquareChar(int x, int y) const {
    ...
    if(characters.length() == 0) { // No splashes in the square -> printing empty
        return ' ';
